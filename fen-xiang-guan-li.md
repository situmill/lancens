...
* 接口相关说明:单个设备uid 不能超过十个分享.选择需要分享的好友id\([好友管理8获取全部好友ID](/../hao-you-guan-li.html)\),  
...
* 接口相关说明:根据XXX组名 ,重新分享组,分享不超过10个人,需要重新选择分享的好友id\([好友管理11获取除屏蔽的好友ID](/../hao-you-guan-li.html)\),拼接数组格式进行分享,比如选择好友添加好友id为1,2,3  需按参数值数组格式, suids:\[1,2,3,\] ,per权限也同理传\(注意字符串需要加双引号引起来,否则会报格式错误问题\)
  \* 
* 请求接口中  \(  -H "token":"token" \) 中 token 为 [用户登录时的token](/../deng-lu-yu-tui-chu.html),用作唯一访问令牌,API接口都需要传到header中
